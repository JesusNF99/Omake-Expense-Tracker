...
import { Save, ArrowLeft, ArrowDownCircle, ArrowUpCircle, Calendar, Tag, FileText, Euro } from 'lucide-react';
...
const EditTransaction = () => {
  ...
  useEffect(() => {
    const fetchTransaction = async () => {
      try {
        ...
        const txs = res.data.content ? res.data.content : (Array.isArray(res.data) ? res.data : []);
        ...
        if (tx) {
          ...
          setFormData({
            ...
            date: rawDate ? String(rawDate).split('T')[0] : new Date().toISOString().split('T')[0]
          });
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Failed to update transaction. Please check your inputs.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (initialLoading) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-cyan-400">
        ...
        <p className="tracking-widest uppercase text-sm font-medium">Loading Transaction...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-cyan-500/30 overflow-x-hidden p-6 relative">
      ...
      <div className="max-w-xl mx-auto relative z-10 w-full">
        ...
        <div className="bg-zinc-900/40 backdrop-blur-xl rounded-3xl border-t border-l border-white/10 p-8 shadow-[0_20px_40px_rgba(6,182,212,0.05)] relative overflow-hidden group">
          ...
          <h1 className="text-2xl font-light text-white mb-2 tracking-wide">Edit Transaction</h1>
          <p className="text-zinc-400 text-sm mb-8">Update the details of your recorded transaction.</p>
